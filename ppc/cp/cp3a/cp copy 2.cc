...
//   return _mm512_permute4f128_ps(x, x, 0b00000001);
// }
inline double4_t swap2(double4_t x) {
  return _mm256_permute2f128_pd(x, x, 0b00000001);
}
inline double4_t swap1(double4_t x) { return _mm256_permute_pd(x, 0b0101); }
...
#pragma omp critical
      { rows[count++] = std::make_tuple(ija, ia, ja); }
    }
  }
  std::sort(rows.begin(), rows.end());
  // std::cout << "na: " << na << "\tcount: " << count << std::endl;

  // for (auto t : rows) {
  //   int ija = std::get<0>(t);
  //   int ia = std::get<1>(t);
  //   int ja = std::get<2>(t);
  //   std::cout << "tuple: " << ija << "\t" << ia << "\t" << ja << std::endl;
  // }
