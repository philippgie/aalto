...
void solver(int n, int m, const int *e, const int *c, int &k, int *s)
{
	...
	struct Edge{
		...
		Edge(int p, int q) : p(p), q(q){}
		bool operator== (const struct Edge& other) const {return ((p==other.p) && (q==other.q));}
	};
	...
	struct hash_fn {
	    ...
	    {
	        std::size_t h1 = std::hash<int>()(edge.p);
	        std::size_t h2 = std::hash<int>()(edge.q);
	 
	        return h1 ^ h2;
	    }
	};

	std::vector<int> edge_order(m); // Order in which edges are selected 
	std::vector<int> x_e(n, 0); 
    //std::unordered_map<Edge, int, hash_fn> e_duplicate; 
    std::unordered_map<Edge, int, hash_fn> y_e; // Use hashmap because of duplicate edges in e

	std::iota(edge_order.begin(), edge_order.end(), 0);
	//std::shuffle(edge_order) // Shuffle to randomly select edges
	...
	for(int i : edge_order) {
		...
		// Check if one of the constraints is already tight
		if(!p_diff || !q_diff)
			continue;

		Edge edge=Edge(p,q);
	
		// Check if y_e is already frozen
		if(y_e[edge])
			continue;

		// Check if constraint becomes tight for q
		if(p_diff>q_diff) {
			y_e[edge]=q_diff;
			x_e.at(p)+=q_diff;
			x_e.at(q)+=q_diff;
		...
		// Check if constraint becomes tight for p
		else if(p_diff<q_diff) {
			y_e[edge]=p_diff;
			x_e.at(p)+=p_diff;
			x_e.at(q)+=p_diff;
			s[k++]=p;
		// Constraint becomes tight for p and q
		} else {
			y_e[edge]=p_diff;
			x_e.at(p)+=p_diff;
			x_e.at(q)+=p_diff;
