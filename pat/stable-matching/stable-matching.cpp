...
//???
	...
	int *s_tmp = new int[n];
	int **pref_pointers = new int*[n];
	int (*b_pref_inv)[2048] = new int[n][2048];
	int ind, best;
	...
	for(int i=0;i<n;i++) {
		unmatched.push(i);
		s_tmp[i] = -1;
		pref_pointers[i] = (int*) a_pref+i*n;
		perm_inv(n, &b_pref[i*n], b_pref_inv[i]);
	}
	...
	while(unmatched.size()) {
		while(unmatched.size()) {
			ind = unmatched.top();
			unmatched.pop();
			int proposee = pref_pointers[ind][0];
			best = s_tmp[proposee];

			if(best==-1) {
				best=ind;
			}
			else if (b_pref_inv[proposee][ind]<b_pref_inv[proposee][best]) {
				unmatched.push(best);
				best=ind;
			} else {
				unmatched.push(ind);
