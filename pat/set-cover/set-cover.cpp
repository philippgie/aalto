...
void solver(int n, int m, const int *p, const int *f, int &k, int *r)
{
	...
	struct Set{
		int m_id;
		int m_size;
		Set(int p, int s) : m_id(p), m_size(s){} 
		bool operator< (const struct Set& other) const {return m_size<other.m_size;}
	};
	k=0;
	if(m==0) 
		return;
	
	int* m_sizes = new int[m]();
	bool* n_covered = new bool[n]();
	std::vector < std::set<int> > n_sets(n);
	std::priority_queue<Set> queue;



	for(int i=0;i<m;i++) {
		for(int j=p[i];j<p[i+1];j++) {
			n_sets.at(f[j]).insert(i);
			m_sizes[i]++;
		}
		queue.emplace(i,m_sizes[i]);
	}
	int u_size = 0;
	while((u_size<n)&&queue.size()) {
		Set s = queue.top();
		queue.pop();
		//std::cout<<s.m_id<<std::endl;
		//std::cout<<s.m_size<<std::endl;
		//std::cout<<m_sizes[s.m_id]<<std::endl;
		if(s.m_size!=m_sizes[s.m_id])
			continue;
		if(s.m_size==0) {
			k=0;
			return;
		...
		r[k++]=s.m_id;
		for(int i=p[s.m_id];i<p[s.m_id+1];i++) {
			if(n_covered[f[i]])
				continue;
			n_covered[f[i]]=true;
			u_size++;
			...
			//std::cout<<":)"<<std::endl;
			for (auto &m : n_sets.at(f[i])){	
				if(m!=s.m_id) {
					m_sizes[m]--;	
					queue.emplace(m,m_sizes[m]);
				}
			}
			m_sizes[s.m_id]=0;
		}	
